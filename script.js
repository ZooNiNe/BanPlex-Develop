...
import {
  ...
  function _getEmptyStateHTML({ icon = 'inbox', title = 'Tidak Ada Data', desc = 'Belum ada data untuk ditampilkan di sini.', action, actionLabel, actionType } = {}) {
    const actionButtonHTML = (action && actionLabel)
        ? `<div class="empty-state-actions">
             <button class="btn btn-primary" data-action="${action}" ${actionType ? `data-type="${actionType}"` : ''}>
...
function _getSkeletonLoaderHTML(pageType) {
    // Gunakan switch untuk memilih skeleton yang sesuai
    switch (pageType) {
        case 'dashboard':
            // Skeleton untuk dashboard sudah bagus, kita pertahankan
            return `
                <div class="skeleton-wrapper">
        ...
        case 'pemasukan':
        case 'log_aktivitas':
            // Halaman-halaman ini memiliki struktur list yang mirip
            return _getSkeletonForListPage();

        case 'pengeluaran':
        case 'absensi':
            // Halaman-halaman ini memiliki struktur form
            const formRow = `
        ...
        default:
            // Fallback jika tidak ada skeleton khusus
...
function createModal(type, data = {}) {
    const isDesktopView = window.matchMedia('(min-width: 600px)').matches;
    const detailPaneTypes = ['dataDetail', 'payment', 'manageMaster', 'editMaster', 'editItem', 'editAttendance', 'manageUsers', 'invoiceItemsDetail', 'billActionsModal'];

    if (isDesktopView && detailPaneTypes.includes(type)) {
        ...
        const title = contentEl.querySelector('.modal-header h4')?.textContent || data.title || '';
        ...
        const detailPaneElement = document.getElementById('detail-pane');
        if (detailPaneElement) {
            attachModalEventListeners(type, data, closeDetailPane, detailPaneElement);
        }
        return; // Hentikan eksekusi agar modal mobile tidak dibuat
    }
  ...
  function getModalContent(type, data) {
    ...
    const modalWithHeader = (title, content, footerContent = '') => {
        const isDesktopView = window.matchMedia('(min-width: 600px)').matches;
        const isTouch = !isDesktopView;
        const footerClass = !isTouch && (footerContent.match(/<button/g) || []).length > 2 ? "modal-footer grid-footer" : "modal-footer";
        const footerHTML = footerContent ? `<div class="${footerClass}">${footerContent}</div>` : '';
  ...
  function attachModalEventListeners(type, data, closeModalFunc, contextElement = document) {
                ...
                if (type === 'manageMaster') {
                    ...
                    createModal('confirmEdit', {
                        onConfirm: () => {
                            ...
                            closeModalFunc(); // Gunakan closeModalFunc agar konsisten
                        }
                    });
                }
            });
    ...
    if (type === 'editItem') {
        ...
        if ($('#material-invoice-form') || $('#edit-item-form[data-type="expense"] #invoice-items-container')) {
            _attachPengeluaranFormListeners('material', contextElement); // Kirim konteks
        }
    }
  ...
  async function renderPemasukanPage() {
      ...
      <div class="content-panel">
          ...
          ${tabs.map((tab, index) => 
            ...
            ${index === 0 ? 'active' : ''}" data-tab="${tab.id}">${tab.label}</button>`).join('')}
          </div>
        ...
        formHTML = `
            <div class="card card-pad desktop-form-layout">
                <form id="pemasukan-form" data-type="termin" data-async="true" method="POST" data-endpoint="/api/incomes" data-success-msg="Pemasukan tersimpan">
                    ...
                    <div class="form-group">
                        <label>Tanggal</label>
                        <input type="date" id="pemasukan-tanggal" value="${todayString}" required>
                    </div>
            ...
            <div class="card card-pad desktop-form-layout">
                <form id="pemasukan-form" data-type="pinjaman" data-async="true" method="POST" data-endpoint="/api/loans" data-success-msg="Pinjaman tersimpan">
                    <div class="form-grid-2col">
                        ...
                        ${createMasterDataSelect('pemasukan-kreditur', 'Kreditur', creditorOptions, '', '')}
                        
                        ${createMasterDataSelect('loan-interest-type', 'Jenis Pinjaman', loanTypeOptions, 'none')}
                    </div>
                    
                    <div class="loan-details hidden full-width">
                        <div class="form-group">
                    ...
                    <div class="form-footer-actions full-width">
                        <button type="button" class="btn btn-secondary" data-action="manage-master" data-type="creditors">
                            <span class="material-symbols-outlined">add</span> Tambah Kreditur
                        </button>
...
// GANTI TOTAL FUNGSI LAMA DENGAN VERSI FINAL YANG LEBIH CERDAS INI
function _initCustomSelects(context = document) {
    ...
    context.querySelectorAll('.custom-select-wrapper').forEach(wrapper => {
        ...
        trigger.addEventListener('click', (e) => {
            ...
            if (window.matchMedia('(min-width: 600px)').matches) {
                ...
                // ▼▼▼ [LOGIKA CERDAS UNTUK POSISI DROPDOWN] ▼▼▼
                clone.style.position = 'fixed';
                clone.style.left = `${rect.left}px`;
                clone.style.width = `${rect.width}px`;
                
                const viewportHeight = window.innerHeight;
                const dropdownHeight = clone.offsetHeight > 250 ? 250 : clone.offsetHeight; // Perkirakan tinggi

                // Cek apakah ada cukup ruang di bawah tombol
                if (rect.bottom + dropdownHeight > viewportHeight) {
                    // Jika tidak cukup, buka ke ATAS
                    clone.style.bottom = `${viewportHeight - rect.top + 4}px`;
                    clone.style.top = 'auto';
                } else {
                    // Jika cukup, buka ke BAWAH (perilaku default)
            ...
            } else { // Logika untuk mobile
                wrapper.classList.add('active');
                if (searchInput) searchInput.focus();
            }
        });

        // Event listener untuk memilih opsi (ini akan berfungsi untuk mobile)
        if (optionsList) {
  ...
  function _getFormPengeluaranHTML(type, categoryOptions, categoryMasterType, categoryLabel, supplierOptions, projectOptions) {
    ...
    <div class="section-header-flex full-width">
        <h5 class="invoice-section-title" style="margin: 0; padding: 0; border: none;">Detail Pengeluaran</h5>
        <div style="display: flex; gap: 0.5rem;">
            <button type="button" class="btn btn-secondary btn-sm" data-action="manage-master" data-type="${categoryMasterType}">
                <span class="material-symbols-outlined">category</span>
                <span>Tambah Kategori</span>
            </button>
            <button type="button" class="btn btn-secondary btn-sm" data-action="manage-master" data-type="suppliers">
    ...
    <div class="card card-pad desktop-form-layout">
        <form id="pengeluaran-form" data-type="${type}" data-async="true" method="POST" data-endpoint="/api/expenses" data-success-msg="Pengeluaran tersimpan">
            ...
            <div class="form-grid-2col">
                ...
                <div class="form-group">
                    <label>Tanggal</label>
                    <input type="date" id="pengeluaran-tanggal" name="pengeluaran-tanggal" value="${todayString}" required>
                </div>
                ...
                ${createMasterDataSelect('expense-supplier', 'Supplier/Penerima', supplierOptions, '', '')}            </div>
          ...
          <div class="card card-pad desktop-form-layout">
              <form id="material-invoice-form" data-type="material" data-async="true" method="POST" data-endpoint="/api/invoices/material" data-success-msg="Faktur material tersimpan">
                  ...
                  <div class="form-grid-2col">
                      ...
                      ${createMasterDataSelect('supplier-id', 'Supplier', supplierOptions, '')}
                      <div class="form-group">
...
container.innerHTML = `
  ...
  function _getManualAttendanceHTML() {
    // Mengatur tanggal default ke hari ini dengan penanganan zona waktu
    const today = new Date();
    const offset = today.getTimezoneOffset();
    const todayLocal = new Date(today.getTime() - (offset*60*1000));
    const todayString = todayLocal.toISOString().split('T')[0];
...
async function _renderManualAttendanceList(dateStr, projectId) {
    ...
    // [PERBAIKAN KUNCI] Cek apakah ini tampilan desktop
    const isDesktop = window.matchMedia('(min-width: 992px)').matches;

    // Siapkan inline style untuk tata letak desktop
    ...
    // Logika event listener tidak perlu diubah, hanya perlu CSS tambahan untuk state :checked
  ...
  async function _renderJurnalHarianView(container) {
    ...
    const listHTML = sortedDays.map(([date, data]) => {
        ...
        // [UI BARU] Gunakan class .jurnal-card
        return `
            <div class="jurnal-card" data-action="view-jurnal-harian" data-date="${date}">
                <div class="jurnal-card-main">
                    <strong class="title">${formattedDate}</strong>
                    <span class="subtitle">${data.workerCount} Pekerja Hadir</span>
                </div>
                <div class="jurnal-card-secondary">
                    <strong class="amount negative">${fmtIDR(data.totalUpah)}</strong>
  ...
  async function handleViewWorkerRecapModal(workerId) {
      ...
      try {
              ...
              <div class="detail-section">
                  <div class="detail-summary-grid">
                      <div class="summary-item">
                          <span class="label">Total Upah Dihasilkan</span>
                          <strong class="value">${fmtIDR(totalEarned)}</strong>
                      </div>
                      <div class="summary-item">
                          <span class="label">Total Telah Dibayar</span>
                          <strong class="value positive">${fmtIDR(totalPaid)}</strong>
                      </div>
                      <div class="summary-item">
                          <span class="label">Sisa Gaji (Tagihan)</span>
                          <strong class="value ${totalUnpaid > 0 ? 'negative' : ''}">${formattedSisaGaji}</strong>
                      </div>
                  </div>
              </div>
          `;
          ...
          const attendanceHistoryHTML = relatedRecords.length > 0 ? relatedRecords.map(rec => {
              ...
              const date = _getJSDate(rec.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long' });
              return `
                  <div class="detail-list-item-card">
                      <div class="item-main">
                          <span class="item-title">${date}</span>
                          <span class="item-subtitle">${project?.projectName || 'N/A'}</span>
                      </div>
                      <strong class="item-secondary">${fmtIDR(rec.totalPay || 0)}</strong>
                  </div>
              `;
          ...
          const content = `
              ${summaryHTML}
              <div class="detail-section">
                  <h5 class="detail-section-title">Riwayat Absensi (Upah Dihasilkan)</h5>
                  ${attendanceHistoryHTML}
              </div>
              <div class="detail-section">
                  <h5 class="detail-section-title">Riwayat Pembayaran Gaji</h5>
                  ${paymentHistoryHTML}
              </div>
          `;          
          hideToast();
  ...
  async function _renderJurnalPerPekerjaView(container) {
      ...
      const listHTML = activeWorkers.map(worker => {
        const profession = appState.professions.find(p => p.id === worker.professionId)?.professionName || 'Tanpa Profesi';
        // [UI BARU] Gunakan class .jurnal-card dan hapus ikon
        return `
            <div class="jurnal-card" data-action="view-worker-recap" data-worker-id="${worker.id}">
                <div class="jurnal-card-main">
                    <strong class="title">${worker.workerName}</strong>
    ...
    async function handleViewJurnalHarianModal(dateStr) {
        ...
        // [GANTI] Gunakan .detail-summary-grid untuk ringkasan
        const summaryHTML = `
            <div class="detail-section">
                <div class="detail-summary-grid">
                    <div class="summary-item">
                        <span class="label">Total Pekerja Hadir</span>
                        <strong class="value">${dayData.workerCount} Orang</strong>
                    </div>
                    <div class="summary-item">
                        <span class="label">Total Beban Upah</span>
                        <strong class="value negative">${fmtIDR(dayData.totalUpah)}</strong>
                    </div>
                </div>
            </div>
        `;
        ...
        const projectSectionsHTML = Object.entries(workersByProject).map(([projectId, records]) => {
            ...
            const workersHTML = records.sort((a, b) => (a.workerName || '').localeCompare(b.workerName || '')).map(rec => {
                ...
                // [GANTI] Gunakan .detail-list-item-card untuk setiap pekerja
                return `
                <div class="detail-list-item-card">
                    <div class="item-main">
                        <strong class="item-title">${workerName}</strong>
                    </div>
            ...
            // [GANTI] Bungkus setiap proyek dalam .detail-section
            return `
                <div class="detail-section">
        ...
        const content = summaryHTML + projectSectionsHTML;
  ...
  function showDetailPane({ title, content, footer }) {
    ...
    detailPane.innerHTML = `
        <div class="detail-pane-header">
            <h4>${title}</h4>
            <button class="btn-icon" data-action="close-detail-pane">
                <span class="material-symbols-outlined">close</span>
            </button>
        </div>
    ...
    if (detailPane) {
        detailPane.innerHTML = `
            <div class="detail-pane-body">
                ${_getEmptyStateHTML({
                    icon: 'inbox_customize', // Ikon yang lebih relevan
...
async function handleOpenBillDetail(billId, expenseId) {
    ...
    } catch (err) { console.warn('Gagal memuat riwayat pembayaran', err); }
    if (bill) {
        try {
        ...
        } catch (e) { console.warn('Gagal memuat antrean pembayaran offline', e); }
    }
    ...
    if (!isViewer() && bill) {
        ...
        const actions = [];
        if (!isPaid) {
            actions.push(`<button class="btn btn-primary" data-action="pay-bill" data-id="${billId}"><span class="material-symbols-outlined">payment</span> Bayar/Cicil</button>`);
        }
        actions.push(`<button class="btn btn-secondary" data-action="edit-item" data-id="${billId}" data-type="bill"><span class="material-symbols-outlined">edit</span> Edit</button>`);
        if (isPaid) {
            actions.push(`<button class="btn btn-secondary" data-action="cetak-kwitansi" data-id="${billId}"><span class="material-symbols-outlined">receipt_long</span> Cetak Kwitansi</button>`);
        }
        actions.push(`<button class="btn btn-danger" data-action="delete-item" data-id="${billId}" data-type="bill"><span class="material-symbols-outlined">delete</span> Hapus</button>`);
        footerContent = actions.join('');
    }
    ...
    if (bill && bill.type === 'gaji') {
        ...
        if (!expenseData) {
            try {
                expenseData = await localDB.expenses.get(targetExpenseId);
            } catch (e) {
                console.warn("Gagal mengambil expense dari localDB:", e);
            }
        }

        if (!expenseData) {
            toast('error', 'Data pengeluaran terkait tidak ditemukan di perangkat.');
            return;
        }
    ...
    if (expenseData && expenseData.status === 'delivery_order' && !isViewer()) {
        footerContent = `<button class="btn btn-primary" data-action="edit-surat-jalan" data-id="${expenseData.id}"><span class="material-symbols-outlined">edit_note</span> Input Harga & Buat Tagihan</button>`;
    }

    if (isTouch) {
        createModal('dataDetail', { title, content, footer: footerContent });
    } else {
        showDetailPane({ title, content, footer: footerContent });
    }
    ...
    createModal('dataDetail', {
        title,
        content: content,
        footer: footerContent, // Properti baru untuk footer
  ...
  function _createSalaryBillDetailContentHTML(bill, payments) {
    ...
    const summaryHTML = `
        <div class="detail-section">
            <div class="detail-summary-grid">
                <div class="summary-item">
                    <span class="label">Total Tagihan Gaji</span>
                    <strong class="value">${fmtIDR(bill.amount)}</strong>
                </div>
                <div class="summary-item">
                    <span class="label">Sudah Dibayar</span>
                    <strong class="value positive">${fmtIDR(bill.paidAmount || 0)}</strong>
                </div>
                <div class="summary-item">
    ...
    const paymentHistoryHTML = (payments && payments.length > 0) ? `
        <div class="detail-section">
            ...
            ${_createPaymentHistoryHTML(payments)}
        </div>
    ` : '';

    let detailsHTML = '';
    // Logika untuk menampilkan rincian per pekerja atau rincian absensi
    if (bill.workerDetails && bill.workerDetails.length > 0) {
        const workerListHTML = bill.workerDetails.map(worker => {
            ...
            const paidByWorker = (payments || []).filter(p => p.workerId === worker.id).reduce((sum, p) => sum + (p.amount || 0), 0);
            ...
            let actionButtons = '';
            // [PERBAIKAN] Tambahkan tombol Cetak Kwitansi jika tagihan sudah lunas
            if (bill.status === 'paid') {
                actionButtons += `
            ...
            // Logika untuk tombol bayar individual tetap ada jika diperlukan
            if (!isViewer() && bill.status === 'unpaid') {
                 actionButtons += `...`; // (jika kamu punya logika tombol bayar per orang, biarkan di sini)
            }            // Gunakan style kartu yang baru
            return `
                <div class="detail-list-item-card">
                    <div class="item-main">
                ...
                <div class="item-secondary" style="display: flex; align-items: center; gap: 0.5rem;">
                    <strong class="value">${fmtIDR(totalForWorker)}</strong>
                    ${actionButtons}
                </div>
            </div>
            `;
        }).join('');
        detailsHTML = `<div class="detail-section"><h5 class="detail-section-title">Rincian Gaji per Pekerja</h5>${workerListHTML}</div>`;
    } else {
        ...
        if (relatedRecords.length > 0) {
            const recordDetailsHTML = relatedRecords.map(rec => {
                // ... (logika yang sudah ada, tidak perlu diubah)
                const project = appState.projects.find(p => p.id === rec.projectId);
                const date = _getJSDate(rec?.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' });
                // Gunakan style kartu yang baru
                return `
                    <div class="detail-list-item-card">
                        <div class="item-main">
            ...
            detailsHTML = `<div class="detail-section"><h5 class="detail-section-title">Rincian Absensi Terkait</h5>${recordDetailsHTML}</div>`;
        }
    }

    return summaryHTML + paymentHistoryHTML + detailsHTML;
}
...
async function _createBillDetailContentHTML(bill, expenseData, payments) {
    ...
    const summaryHTML = `
        <div class="detail-section">
            <div class="detail-summary-grid">
                ...
                <div class="summary-item">
                    <span class="label">Sudah Dibayar</span>
                    <strong class="value positive">${fmtIDR(bill ? bill.paidAmount || 0 : expenseData.amount)}</strong>
                </div>
                <div class="summary-item">
                    <span class="label">Sisa Tagihan</span>
                    <strong class="value ${remainingAmount > 0 ? 'negative' : ''}">${fmtIDR(remainingAmount)}</strong>
    ...
    const projectDetailsHTML = project ? `
        <div class="detail-section">
            <h5 class="detail-section-title">Detail Proyek</h5>
            <dl class="detail-list">
    ...
    const invoiceItemsHTML = (expenseData && expenseData.type === 'material') ? `
        <div class="detail-section">
             <button class="btn btn-secondary" data-action="view-invoice-items" data-id="${expenseData.id}" style="width: 100%;">
    ...
    return summaryHTML + projectDetailsHTML + invoiceItemsHTML + paymentHistoryHTML + attachmentsHTML + commentsHTML;
}
...
function _createCommentsSectionHTML(parentId, parentType) {
    try {
        ...
        // Buat HTML untuk setiap item komentar menggunakan struktur baru
        ...
        context.querySelectorAll('.btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (action === 'pay_partial') {
                    if (window.matchMedia('(min-width: 600px)').matches) {
                        // Di desktop, kita tidak perlu menutup panel, cukup buka panel baru
  ...
  async function handleCetakKwitansi(billId) {
    ...
    if (bill.type === 'gaji') {
        if (bill.workerDetails && bill.workerDetails.length === 1) {
            ...
            recipientName = "Beberapa Pekerja"; // Atau teks lain yang sesuai
    ...
    const contentHTML = `<div id="kwitansi-printable-area">${_getKwitansiHTML(kwitansiData)}</div>`;
    ...
    createModal('dataDetail', {
        ...
        content: contentHTML,
        footer: footerHTML // Teruskan footer sebagai properti terpisah
    });

    hideToast();

    const isDesktop = window.matchMedia('(min-width: 600px)').matches;
    const context = isDesktop ? $('#detail-pane') : $('#dataDetail-modal');
    
    if (context) {
        $('#download-kwitansi-img-btn', context).addEventListener('click', () => _downloadKwitansiAsImage(kwitansiData));
        $('#download-kwitansi-btn', context).addEventListener('click', () => _downloadKwitansiAsPDF(kwitansiData));
    }
}
...
async function handleCetakKwitansiIndividu(dataset) {
    ...
    const kwitansiData = {
        ...
        tanggal: bill.paidAt ? _getJSDate(bill.paidAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }),
    ...
    createModal('dataDetail', {
        title: 'Pratinjau Kwitansi Individual',
        content: contentHTML,
        footer: footerHTML
    });
    ...
    const context = document.querySelector('#dataDetail-modal') || document.getElementById('detail-pane');
    if (context) {
        $('#download-kwitansi-img-btn', context).addEventListener('click', () => _downloadKwitansiAsImage(kwitansiData));
        $('#download-kwitansi-btn', context).addEventListener('click', () => _downloadKwitansiAsPDF(kwitansiData));
    }
...
async function handleDeleteMasterItem(id, type) {
  ...
  async function handleEditItem(id, type) {
    ...
    let originalType = type; // Simpan tipe asli untuk pencarian di localDB

    // ▼▼▼ BAGIAN YANG DIUBAH ▼▼▼

    // --- Langkah 1: Cari data di appState (memori) terlebih dahulu ---
    if (type === 'bill') {
    ...
    // --- Langkah 2: Jika tidak ditemukan di appState, cari di localDB (database lokal) ---
    if (!item) {
        try {
            console.log(`Item tidak ditemukan di appState, mencari di localDB untuk tipe: ${originalType}`);
            if (originalType === 'bill') item = await localDB.bills.get(id);
            else if (originalType === 'expense') item = await localDB.expenses.get(id);
            else if (originalType === 'termin') item = await localDB.incomes.get(id);
            else if (originalType === 'pinjaman') item = await localDB.funding_sources.get(id);
        } catch (e) {
            console.error("Gagal mengambil data dari localDB:", e);
        }
    }
    
    // Logika untuk mengubah 'type' jika 'bill' adalah 'expense'
    if (type === 'bill' && item) {
        if (item.expenseId) {
            type = 'expense'; 
            // Cari data expense-nya, baik di state maupun localDB
            item = appState.expenses.find(e => e.id === item.expenseId) || await localDB.expenses.get(item.expenseId);
        } else if (item.type === 'fee') {
            type = 'fee_bill';
        }
    }
    
    // --- Langkah 3: Jika masih tidak ada, tampilkan pesan error ---
    if (!item) {
        toast('error', 'Data tidak ditemukan untuk diedit.');
        return;
    }
  ...
  async function handleUpdateItem(form) {
      ...
      try {
          ...
          // Bagian ini (switch case) tidak berubah, tugasnya menyiapkan data
          switch (type) {
              case 'termin':
          ...
          // Tentukan tabel mana saja yang terlibat dalam transaksi secara dinamis
          const tablesInTransaction = [table];
          if (type === 'expense') {
              // Jika yang diedit adalah expense, kita juga butuh akses ke tabel bills
              tablesInTransaction.push(localDB.bills);
          }
  
          // Jalankan transaksi dengan daftar tabel yang sudah benar
          await localDB.transaction('rw', tablesInTransaction, async () => {
              // 1. Update data utama (termin, pinjaman, expense, dll)
              await table.where('id').equals(id).modify({ ...dataToUpdate, needsSync: 1 });
              
              // 2. Jika yang di-update adalah 'expense', update juga data 'bill' yang terkait
              if (type === 'expense') {
                  ...
                  if (relatedBill) {
                      ...
                      await localDB.bills.update(relatedBill.localId, billUpdateData);
                  }
              }
          });
          // [PERBAIKAN SELESAI]
          ...
          onConfirm: async () => {
              ...
              try {
                  ...
                  try {
                      ...
                      // Blok catch ini sekarang akan menangani kegagalan jaringan DAN server dev statis
                      // Sehingga fallback ke Firestore akan berjalan dengan benar
                  }
                  ...
                  _logActivity(`Menghapus Data ${type}`, {
                      docId: id,
                      description: item?.description || item?.amount || 'N/A' // Perbaikan untuk Error #2 juga ada di sini
                  });
  ...
  function attachEventListeners() {
      ...
      document.body.addEventListener('click', (e) => {
          ...
          const actions = {
              ...
              'open-detail': async () => { // <-- Tambahkan 'async' di sini
                if (appState.activePage === 'pemasukan') {
                    const card = target.closest('.dense-list-item');
                    if (!card) return;

                    const { id: itemId, type: itemType } = card.dataset;
                    if (!itemId) return;

                    let item = itemType === 'termin'
                        ? appState.incomes.find(i => i.id === itemId)
                        : appState.fundingSources.find(i => i.id === itemId);

                    if (!item) {
                        try {
                            item = itemType === 'termin'
                                ? await localDB.incomes.get(itemId)
                                : await localDB.funding_sources.get(itemId);
                        } catch (e) {
                            console.error("Gagal mengambil detail pemasukan dari localDB:", e);
                        }
                    }
                    ...
                    if (item) {
                        ...
                        const actions = [];
                        if (itemType === 'pinjaman' && !isPaid) {
                            actions.push(`<button class="btn btn-primary" data-action="pay-loan" data-id="${itemId}" data-type="pinjaman"><span class="material-symbols-outlined">payment</span> Bayar Cicilan</button>`);
                        }
                        // Tombol Edit
                        actions.push(`<button class="btn btn-secondary" data-action="edit-item" data-id="${itemId}" data-type="${itemType}"><span class="material-symbols-outlined">edit</span> Edit</button>`);
                        // Tombol Hapus
                        actions.push(`<button class="btn btn-danger" data-action="delete-item" data-id="${itemId}" data-type="${itemType}"><span class="material-symbols-outlined">delete</span> Hapus</button>`);
                        
                        footerContent = actions.join('');

                        createModal('dataDetail', {
                            title: (itemType === 'pinjaman') ? 'Kartu Data Pinjaman' : 'Detail Pemasukan',
                            content: _createDetailContentHTML(item, itemType || 'termin'),
                            footer: footerContent // <-- Teruskan tombol ke modal/detail pane
                        });
                    } else {
                        toast('error', 'Gagal menemukan detail data pemasukan.');
                    }
                }
            },
            ...
            'open-actions': () => {
              ...
              'pay-bill': () => handlePayBillModal(target.dataset.id),
              'open-bill-detail': () => {
                  const itemEl = target.closest('[data-id]');
                  const billId = itemEl ? itemEl.dataset.id : null;
                  const expenseIdEl = target.closest('[data-expense-id]');
              ...
              'cetak-kwitansi-individu': () => handleCetakKwitansiIndividu(target.dataset),              'pay-individual-salary': () => handlePayIndividualSalaryModal(target.dataset),
              ...
              'add-new-material': () => {
                  ...
                  const wrapper = row?.querySelector('.custom-select-wrapper'); // Diperbaiki dari autocomplete-wrapper
